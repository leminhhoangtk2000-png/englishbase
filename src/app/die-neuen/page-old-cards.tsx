    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="text-sm font-medium text-slate-500 tracking-wider uppercase mb-2">
            5 BIG THINGS
          </div>
          ...
          <div className="grid gap-6">
            {displayArticles.length === 0 ? (
              ...
              displayArticles.map((article, index) => {
                // Derive category from source or keywords in title/content
                const getArticleCategory = (article: NewsArticle) => {
                  const text = (article.title + ' ' + article.excerpt).toLowerCase();
                  if (text.includes('politik') || text.includes('bundestag') || text.includes('regierung')) return 'Politik';
                  if (text.includes('wirtschaft') || text.includes('unternehmen') || text.includes('markt')) return 'Wirtschaft';
                  if (text.includes('technologie') || text.includes('digital') || text.includes('ki')) return 'Technologie';
                  if (text.includes('sport') || text.includes('fußball') || text.includes('olympia')) return 'Sport';
                  if (text.includes('kultur') || text.includes('kunst') || text.includes('musik')) return 'Kultur';
                  if (text.includes('gesellschaft') || text.includes('sozial')) return 'Gesellschaft';
                  if (text.includes('wissenschaft') || text.includes('forschung')) return 'Wissenschaft';
                  return 'General';
                };
                ...
                const categoryColors = {
                  'Politik': 'bg-red-50 border-red-200',
                  'Wirtschaft': 'bg-green-50 border-green-200',
                  'Technologie': 'bg-blue-50 border-blue-200',
                  'Sport': 'bg-orange-50 border-orange-200',
                  'Kultur': 'bg-purple-50 border-purple-200',
                  'Gesellschaft': 'bg-pink-50 border-pink-200',
                  'Wissenschaft': 'bg-teal-50 border-teal-200',
                ...
                const categoryBadgeColors = {
                  ...
                  'Gesellschaft': 'bg-pink-100 text-pink-700',
                ...
                const cardBg = index === 0 
                  ...
                  : `${categoryColors[articleCategory as keyof typeof categoryColors]} border-l-4`;
                
                const badgeColor = categoryBadgeColors[articleCategory as keyof typeof categoryBadgeColors];
