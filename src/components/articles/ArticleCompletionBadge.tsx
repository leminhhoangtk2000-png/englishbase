...
interface ArticleCompletionBadgeProps {
  articleId: string;
...
export function ArticleCompletionBadge({ 
  articleId, 
  variant = 'badge',
  className = '',
  onCompletionChange
}: ArticleCompletionBadgeProps) {
  const { completion, loading, marking, markCompleted, unmarkCompleted } = useArticleCompletion(articleId);
        ...
        <Tooltip>
          ...
          <TooltipContent>
            ...
            {completion.attempts && completion.attempts > 1 && (
              <p className="text-xs text-gray-400">Đã đọc {completion.attempts} lần</p>
            )}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
  ...
  // Button variant (for article pages)
  if (variant === 'button') {
    return (
      <Button
        onClick={handleToggle}
    ...
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipContent>
          <p className="text-xs">
            {completion.completed 
              ? `Đã hoàn thành ${completion.attempts || 1} lần`
