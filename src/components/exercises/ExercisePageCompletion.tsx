...
interface ExercisePageCompletionProps {
  exerciseId: string;
  /**
   * Minimum time (in seconds) user must spend before auto-completion suggestion
   * Default: 60 seconds (1 minute)
   */
  minTimeForCompletion?: number;
  /**
   * Auto-mark as completed after scrolling to bottom
   * Default: false
   */
...
export function ExercisePageCompletion({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollPercentage >= 0.9 && !hasScrolledToBottom) {
        ...
        if (autoMarkOnScroll && !completion.completed && timeSpent >= minTimeForCompletion) {
          handleAutoComplete();
        }
      }
    };
  ...
  }, [autoMarkOnScroll, completion.completed, timeSpent, minTimeForCompletion, hasScrolledToBottom]);
